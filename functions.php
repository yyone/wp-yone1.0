...
//		wp_register_script('jquery', get_bloginfo('template_directory').'/js/jquery-1.9.1.min.js');
		...
		if(is_single()) {
			wp_register_script('zenback', get_bloginfo('template_directory').'/js/zenback.min.js', false, "", true);
			wp_enqueue_script('zenback');
		}
...
//add_action('wp_print_scripts', 'add_script_header');
...
register_sidebar(array(
	...
	'description' => 'yone用のウィジェットエリア',
	'before_widget' => '<div class="right_box">' ."\n".
...
function mytheme_comment($comment, $args, $depth) {
	...
	<div class="comments-context" id="comment-<?php comment_ID(); ?>">
		<div class="author">
			<?php echo get_avatar($comment,$size='60'); ?>
			<?php printf(__('<span class="fn">%s</span>'), get_comment_author_link()) ?>
			<br />
			<span class="comment-date">
				<a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ) ?>"><?php printf(__('%1$s'), get_comment_date('Y/m/d')) ?></a>
 ...
 *
 * content:
     create new widget copyed original [recent post widget].
 ...
 *
 * content:
     create new widget copyed original [collapsArch widget].
 */
